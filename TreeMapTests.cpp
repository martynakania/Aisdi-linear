...
BOOST_AUTO_TEST_CASE_TEMPLATE(Begins_parent_is_nullptr,//!!MOJ
                              K,
                              TestedKeyTypes)
{
  Map<K> map;
  ...
  BOOST_CHECK(map.begin().getParent() == nullptr);
  BOOST_CHECK(map.cbegin().getParent() == nullptr);
}
...
BOOST_AUTO_TEST_CASE_TEMPLATE(GivenIterator_WhenAddingFewItems_ThenBeginIsTheOneWithTheLowestKey,
                              K,
                              TestedKeyTypes)
{
  Map<K> map = { { 42, "Chuck" }, { 27, "Bob" },{ 2, "Lis" },{ 20, "Swinka" } };

  auto it = map.begin();

  BOOST_CHECK_EQUAL(it->first, 2);
  BOOST_CHECK_EQUAL(it->second, "Lis");
}
BOOST_AUTO_TEST_CASE_TEMPLATE(GivenEndIterator_WhenDecrementing_ThenCorectOrder,//!!my
                              K,
                              TestedKeyTypes)
{
  Map<K> map = { { 42, "Chuck" }, { 27, "Bob" }, { 2, "Lis" }, { 20, "Swinka" } };
  ...
  auto it = map.end();
  BOOST_CHECK_EQUAL((--it)->first, 42);
  BOOST_CHECK_EQUAL((--it)->second, "Bob" );
  BOOST_CHECK_EQUAL((--it)->first, 20 );
  it--;
  BOOST_CHECK_EQUAL( it->second, "Lis" );
}
BOOST_AUTO_TEST_CASE_TEMPLATE(GivenEndIterator_WhenIncrementing_ThenCorectOrder,//!!my
                              K,
                              TestedKeyTypes)
{
  Map<K> map = { { 42, "Chuck" }, { 27, "Bob" }, { 2, "Lis" }, { 20, "Swinka" } };
  ...
  BOOST_CHECK_EQUAL((++it)->second, "Bob");
  it++;
  BOOST_CHECK_EQUAL(it->first, 42);//+3
  BOOST_CHECK(++it == map.end());
}
